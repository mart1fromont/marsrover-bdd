...
up:
	docker run -it -d --env-file ./docker/xdebug.env --name ulco-marsrover-bdd -v $(shell pwd):/app -v $(shell pwd)/docker/xdebug.ini:/usr/local/etc/php/conf.d/docker-php-ext-xdebug-20.ini:ro -w=/app ulco
